...
def AA_saveParams(genuine_matrix, imposter_matrix, measures, scale):
    color = [
            'r',
            'g',
            'c',
    ...
    for feature_type in range(len(genuine_matrix)):
        title = feature_type
        # feature types are (in order): geom, distance, orientation, fusion
        performance = AA_threshold_performance(genuine_matrix,feature_type, imposter_matrix, scale)
        y_TAR = []
        y_primFAR = []
        y_centroidFAR = []
        y_meanshapeFAR = []
        for i in range(len(performance)):
            y_TAR.append(get_TAR(performance[i][1], performance[i][2]))
            y_primFAR.append(getFAR( performance[i][3], performance[i][4]))
            y_centroidFAR.append(getFAR( performance[i][5], performance[i][6] ))
            y_meanshapeFAR.append(getFAR( performance[i][7], performance[i][8] ))

        print(y_primFAR)
        plt.plot(y_primFAR, y_TAR, color[feature_type], label=feature_type)
        plt.xlabel('primFAR')
        plt.ylabel('TAR')
        plt.xscale('log')
        plt.title('ROC - '+ str(title))
        plt.legend()
        plt.savefig( './figures/' + 'ROC_'+ str(title) + '.png')
        plt.close()
...
def AA_threshold_performance(genuine_matrix, feature_type, imposter_matrix, scale):
    ...
    for i, thresh in enumerate(np.linspace(0.0, 1.0, scale)):
        print('...................')
        tp = sum([sum(1 for i in genuine_matrix[feature_type][pers] if i < thresh) for pers in range(len(genuine_matrix[feature_type]))])
        fn = len(genuine_matrix[feature_type][0])*len(genuine_matrix[feature_type]) - tp

        fp_prim = sum(1 for i in imposter_matrix[0][feature_type] if i < thresh)
        tn_prim = len(imposter_matrix[0][feature_type]) - fp_prim
        print(fp_prim, tn_prim , ' , :  ', imposter_matrix[0][feature_type], thresh)

        fp_centroids = sum(1 for i in imposter_matrix[1][feature_type] if i < thresh)
        tn_centroids = len(imposter_matrix[1][feature_type]) - fp_centroids
        print(fp_centroids, tn_centroids , ' , :  ', imposter_matrix[1][feature_type], thresh)

        fp_mean_shape = sum(1 for i in imposter_matrix[2][feature_type] if i < thresh)
        tn_mean_shape =  len(imposter_matrix[2][feature_type]) - fp_mean_shape
        print(fp_mean_shape, tn_mean_shape,  ' , :  ', imposter_matrix[2][feature_type], thresh)

        performance.append([thresh, tp, fn, fp_prim, tn_prim, fp_centroids, tn_centroids, fp_mean_shape, tn_mean_shape ] )
