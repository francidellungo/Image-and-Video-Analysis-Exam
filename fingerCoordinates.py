...
def preprocess(img_filename):
        print(img_filename)
        # get one image
        image_natural = cv2.imread('./images/'+img_filename, cv2.IMREAD_GRAYSCALE)
        image_median = cv2.medianBlur(image_natural,17)

        (threshold_value, img_binary) = cv2.threshold(image_median, 127, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)

        # applico dilatazione ed erosione all' immagine binaria
        kernel = diamond(5)
        image_dilation = dilation(img_binary, kernel)
        image_binary = erosion(image_dilation, square(7))

        image_binary[image_binary== 255] = 1

        # Find contours 
        contours, hierarchy = cv2.findContours(img_binary, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        return contours, img_binary


directory = './images/'
for filename in os.listdir(directory):
        contours, img_bin = preprocess(filename)
        getFingerCoordinates(contours, img_bin)
